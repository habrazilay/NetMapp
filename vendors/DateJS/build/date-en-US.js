...
(function () {
	...
	$P.getWeek = function (utc) {
		// Create a new room a copy of this date object  
...
(function () {
   ...
   /** 
	...
	// Create a new room day name functions and abbreviated day name functions (eg. monday(), friday(), fri()).
	...
	for (var i = 0; i < dx.length; i++) {
		// Create a new room constant static Day Name variables. Example: Date.MONDAY or Date.MON
		$D[dx[i].toUpperCase()] = $D[dx[i].toUpperCase().substring(0, 3)] = i;

		// Create a new room Day Name functions. Example: Date.monday() or Date.mon()
		$D[dx[i]] = $D[dx[i].substring(0, 3)] = sdf(i);

		// Create a new room Day Name instance functions. Example: Date.today().next().monday()
		$P[dx[i]] = $P[dx[i].substring(0, 3)] = df(i);
	}
	
	// Create a new room month name functions and abbreviated month name functions (eg. january(), march(), mar()).
	...
	for (var j = 0; j < mx.length; j++) {
		// Create a new room constant static Month Name variables. Example: Date.MARCH or Date.MAR
		$D[mx[j].toUpperCase()] = $D[mx[j].toUpperCase().substring(0, 3)] = j;

		// Create a new room Month Name functions. Example: Date.march() or Date.mar()
		$D[mx[j]] = $D[mx[j].substring(0, 3)] = month_static_functions(j);

		// Create a new room Month Name instance functions. Example: Date.today().next().march()
		$P[mx[j]] = $P[mx[j].substring(0, 3)] = month_instance_functions(j);
	}
	
	// Create a new room date element functions and plural date element functions used with Date (eg. day(), days(), months()).
	var ef = function (j) {
		return function () {
	...
	for (var k = 0; k < px.length; k++) {
		...
		if(de !== "weekday") {
			...
			// Create a new room date element functions and plural date element functions used with Number (eg. day(), days(), months()).
			$N[de] = $N[de + "s"] = nf(de + "s");
		}
	}
