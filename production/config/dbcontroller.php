...
class DBController {   
	...
	function selectDB($db) {
	    require_once($_SERVER['DOCUMENT_ROOT']."/NetMapp/production/config/set_mysql_server.php");
	    if(strcmp($db,DB_MULTI_SCHEMA) !== 0)
	...
	function prepareAndRunQuery($query,$db,$type,$formatStr,...$args) {
		$errors = array();
		$this->write_log_sql("Starting to execute an SQL query.".PHP_EOL."\tQuery: '".$query."' , ".PHP_EOL."\tDB: '".$db."' , ".PHP_EOL."\tformatStr: '".$formatStr."', ".PHP_EOL."\tArgs: '".print_r($args,true)."' ");
